...
timenow = 0
stage = 0
cap = cv2.VideoCapture(0) #external cam
# resize the cam
...
detector = mp.handDetector(detectionCon=0)
tipIds = [4, 8, 12, 16, 20]
...
while True:
    ...
    # print(lmList)
    cv2.putText(frame, str(my_score), (0, 150), cv2.FONT_HERSHEY_PLAIN, 6, (100, 0, 255), 8)
    cv2.putText(frame, str(pc_score), (150, 150), cv2.FONT_HERSHEY_PLAIN, 6, (100, 0, 255), 8)
    cv2.putText(frame, '-', (80, 140), cv2.FONT_HERSHEY_PLAIN, 4, (100, 0, 255), 5)
    if len(lmList) != 0:
        fingers = []
        # Thumb
        ...
        if totalFingers == 5:
            ...
            frame = cvzone.overlayPNG(frame, overlayList[2], [0, 190])
            my_choice = 2
    ...
    if (timenow < 5):
        # frame[0:289, 1080:1280] = random.choice(overlayList)
        frame = cvzone.overlayPNG(frame, random.choice(overlayList), [400, 190])
        ...
        frame = cvzone.overlayPNG(frame, overlayList[pc_choice], [400, 190])
    else:
        # frame[0:289, 1080:1280] = overlayList[pc_choice]
        frame = cvzone.overlayPNG(frame, overlayList[pc_choice], [400, 190])
    ...
    if (stage == 2) & (timenow < 10):
        if (my_score < 3) & (pc_score < 3):
            cv2.putText(frame, result, (10, 50), cv2.FONT_HERSHEY_PLAIN, 4, (200, 0, 0), 4)
        elif (my_score == 3):
